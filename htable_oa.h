...
#define DEF_PHTABLE_OA_STRUCT(tname, K, V)\
typedef struct{\
	size_t size;\
	size_t occupied;\
	V * data;\
...
#define DEF_PHTABLE_OA_CONSTRUCTOR(tname, K, V)\
static int tname ## _create(tname * self, size_t size,\
				size_t (*hash_func)(size_t table_sz, const K* key))\
{\
	self->size = size;\
...
#define DEF_PHTABLE_OA_DESTRUCTOR(tname, K, V)\
static void tname ## _destroy(tname * self)\
{\
	free(self->data);\
	free(self->keys);\
...
#define DEF_PHTABLE_OA_IS_ALLOCATED(tname, K, V)\
static int tname ## _is_allocated(tname * self, size_t i)\
{\
	size_t epbb = sizeof(size_t) * 8;\
	return !!(self->free_bitmap[i / epbb] & ( ((size_t)1) << (epbb - 1 - i % epbb)));\
}
#define DEF_PHTABLE_OA_SET_ALLOCATED(tname, K, V)\
static void tname ## _set_allocated(tname * self, size_t i, int state)\
{\
	size_t epbb = sizeof(size_t) * 8;\
	if(state)\
...
#define DEF_PHTABLE_OA_INSERT(tname, K, V)\
static int tname ## _rebuild(tname * self, size_t new_sz); /* rebuild and insert depend on each other, so forward declare*/\
static V * tname ## _insert(tname * self, K key, V el)\
{\
	if(self->occupied >= self->size * HTABLE_OA_RATIO)\
...
#define DEF_PHTABLE_OA_FIND(tname, K, V)\
static V * tname ## _find(tname * self, const K key)\
{\
	size_t h = self->hash_func(self->size, (const K*)&key);\
	size_t i;\
...
#define DEF_PHTABLE_OA_DELETE(tname, K, V)\
static void tname ## _delete(tname * self, K key)\
{\
	size_t h = self->hash_func(self->size, (const K*)&key);\
	size_t i;\
...
#define DEF_PHTABLE_OA_REBUILD(tname, K, V)\
static int tname ## _rebuild(tname * self, size_t new_sz)\
{\
	size_t old_size = self->size;\
	self->size = new_sz;\
...
#define DEF_PHTABLE_OA(tname, K, V)\
DEF_PHTABLE_OA_STRUCT(tname, K, V)\
DEF_PHTABLE_OA_IS_ALLOCATED(tname, K, V)\
DEF_PHTABLE_OA_SET_ALLOCATED(tname, K, V)\
DEF_PHTABLE_OA_CONSTRUCTOR(tname, K, V)\
DEF_PHTABLE_OA_DESTRUCTOR(tname, K, V)\
DEF_PHTABLE_OA_INSERT(tname, K, V)\
DEF_PHTABLE_OA_FIND(tname, K, V)\
DEF_PHTABLE_OA_DELETE(tname, K, V)\
DEF_PHTABLE_OA_REBUILD(tname, K, V)
